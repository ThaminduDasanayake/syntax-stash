...
function getImageDataUri(base64String: string): string | null {
  ...
  if (trimmed.startsWith("/9j/")) {
    ...
    trimmed.startsWith("PD94bWwgdmVyc2lvbg==") ||
    trimmed.startsWith("PHN2ZyB4bWxucz0=")
  ) {
    imageType = "svg+xml";
  }
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
        ...
        <div className="space-y-3">
          ...
          <p className="text-muted-foreground text-xs">
            Paste the Base64 string with or without the data URI scheme. If the scheme is missing, it will be auto-detected based on the image format.
          </p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <Card className="bg-background overflow-hidden">
            <CardContent className="flex items-center justify-center min-h-[400px] p-4">
              {base64Input.trim() === "" ? (
      ...
      <div className="bg-muted/30 rounded-lg border border-border p-4 space-y-3 mt-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs text-muted-foreground">
          <div className="space-y-2">
            <p>
              <span className="text-foreground font-semibold">Data URI Scheme:</span> Base64 images are typically prefixed with a scheme like <code className="bg-background px-1 py-0.5 rounded text-[10px]">data:image/png;base64,</code>
            </p>
            <p>
              <span className="text-foreground font-semibold">Auto-Detection:</span> This tool auto-detects missing schemes and infers the image format from the Base64 data.
            </p>
          </div>
          <div className="space-y-2">
            <p>
              <span className="text-foreground font-semibold">Supported Formats:</span> PNG, JPEG, GIF, WebP, SVG+XML
            </p>
            <p>
              <span className="text-foreground font-semibold">Local Processing:</span> All decoding happens in your browser with zero server requests.
            </p>
          </div>
        </div>
      </div>
