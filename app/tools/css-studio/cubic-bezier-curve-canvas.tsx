...
type Props = {
  ...
  p2x: number;
  p2y: number;
  onChangeAction: (field: "p1x" | "p1y" | "p2x" | "p2y", value: number) => void;
};
...
export function CurveCanvas({ onChangeAction, p1x, p1y, p2x, p2y }: Props) {
  ...
  const onPointerMove = (e: React.PointerEvent<SVGSVGElement>) => {
    ...
    if (dragging.current === "p1") {
      onChangeAction("p1x", x);
      onChangeAction("p1y", y);
    } else {
      onChangeAction("p2x", x);
      onChangeAction("p2y", y);
    }
  };
  ...
  return (
    ...
    >
      ...
      <text x={4} y={PAD} fontSize={9} fill="currentColor" fillOpacity={0.4} textAnchor="middle">
        1
      </text>
    </svg>
  );
