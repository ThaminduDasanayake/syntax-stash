...
import {
  ...
  R,
  V_GAP,
} from "@/app/tools/regex-studio/constants.ts";
import { Rendered } from "@/app/tools/regex-studio/types.ts";
...
function renderSequence(children: Rendered[]): Rendered {
  if (children.length === 0) return pill("ε", COLORS.fallback);
...
function renderDisjunction(children: Rendered[]): Rendered {
  if (children.length === 0) return pill("∅", COLORS.fallback);
  if (children.length === 1) return children[0];
    ...
    el: (key) => (
      <g key={key}>
        ...
        <line
          ...
          y1={railY}
          x2={0}
          y2={offsets[children.length - 1] + children[children.length - 1].railY}
          stroke="#475569"
          strokeWidth={1.5}
        />
        <line
...
function wrapGroup(inner: Rendered, label: string, color: typeof COLORS.group): Rendered {
  const labelH = 14;
  const borderPad = 6;
...
export function renderNode(node: RootNode): Rendered {
  switch (node.type) {
    ...
    case "value": {
      ...
      return pill(`"${ch}"`, COLORS.literal);
    }

    case "dot":
      return pill(".", COLORS.literal);

    case "characterClassEscape":
      return pill(`\\${node.value}`, COLORS.charClass);

    case "characterClass": {
      const content = formatCharClassBody(node.body as unknown as CharacterClassBody[]);
      const label = node.negative ? `[^${content}]` : `[${content}]`;
      return pill(label, COLORS.charClass);
    }
...
export function RailroadDiagram({ ast }: { ast: RootNode }) {
  ...
  return (
    ...
    >
      ...
      <g transform={`translate(${MARGIN},${MARGIN})`}>{result.el("root")}</g>
    </svg>
  );
}
