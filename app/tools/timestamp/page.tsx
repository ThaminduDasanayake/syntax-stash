      ...
      <div className="space-y-8">
        ...
        <div className="space-y-3">
          ...
          <p className="text-muted-foreground text-xs">
            Accepts 10-digit seconds (e.g., 1713008400) or 13-digit milliseconds (e.g., 1713008400000)
          </p>
        </div>
            ...
            <CardContent className="space-y-3">
              ...
              {input.trim() && localTime !== "Invalid Date" && (
                <div className="text-muted-foreground text-xs pt-2 border-t border-border">
                  <p>Timestamp: {input}</p>
                  <p>Milliseconds: {Math.floor(parseFloat(input) * (input.trim().split(".")[0].length <= 10 ? 1000 : 1))}</p>
                </div>
              )}
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-3">
              ...
              <div className="space-y-2">
                {input.trim() ? (
                  ...
                  <p className="text-muted-foreground text-sm">
                    Enter a timestamp to see UTC time
                  </p>
                )}
              </div>
              {input.trim() && utcTime !== "Invalid Date" && (
                <div className="text-muted-foreground text-xs pt-2 border-t border-border">
                  <p>ISO String: {new Date(Math.floor(parseFloat(input) * (input.trim().split(".")[0].length <= 10 ? 1000 : 1))).toISOString()}</p>
                </div>
              )}
            </CardContent>
          </Card>
        ...
        <div className="bg-muted/30 rounded-lg border border-border p-4 space-y-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs text-muted-foreground font-mono">
            <div>
