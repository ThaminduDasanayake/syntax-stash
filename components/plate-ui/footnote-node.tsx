...
const getNavigationAttributes = (
  ...
  ...attributes,
  "data-nav-cycle": navigationHighlight ? String(navigationHighlight.cycle) : undefined,
  "data-nav-highlight": navigationHighlight?.variant,
  "data-nav-pulse": navigationHighlight ? String(navigationHighlight.pulse) : undefined,
...
const getReferenceContextLabel = (editor: PlateEditor, path: Path, index: number) => {
...
export function FootnoteReferenceElement(props: PlateElementProps<TFootnoteElement>) {
  ...
  const fallbackPreviewText =
    identifier && footnoteApi ? footnoteApi.definitionText({ identifier }) : undefined;
      ...
      <HoverCard open={hoverOpen} onOpenChange={setHoverOpen} openDelay={150}>
        <HoverCardTrigger asChild>
          <button
            type="button"
            className={cn(
              "text-primary focus:ring-ring cursor-pointer rounded-xs text-xs font-medium group-data-[nav-target=true]/footnote-ref:bg-(--color-highlight) focus:ring-2 focus:ring-offset-1",
        ...
        {previewText ? (
          ...
          <HoverCardContent className="w-80">
            <div className="space-y-2">
              {isResolved ? (
                <div className="text-sm leading-relaxed">No preview available.</div>
              ) : (
                <Button
                  type="button"
                  variant="outline"
  ...
  const definitionState = useEditorSelector(() => {
    ...
    const referenceItems =
      !isDuplicateDefinition && identifier
        ? footnoteApi.references({ identifier }).map((entry: any, index: number) => ({
            index,
  ...
  return (
    ...
    >
      <div contentEditable={false}>
        {isDuplicateDefinition ? (
          <div className="min-w-3 text-xs text-amber-700 tabular-nums">{identifier}</div>
        ) : (
          <Popover open={referencePickerOpen} onOpenChange={setReferencePickerOpen}>
            <PopoverAnchor asChild>
              <button
                ...
                className="text-muted-foreground hover:text-foreground focus:ring-ring min-w-3 cursor-pointer rounded-xs text-xs tabular-nums underline-offset-2 focus:ring-2 focus:ring-offset-1"
                onClick={(event) => {
                  event.preventDefault();
                  event.stopPropagation();
                }}
        ...
        <InlineComboboxContent className="my-1.5 w-72">
          ...
          <InlineComboboxGroup>
            {showCreateOption && (!query || numericQuery) ? (
              ...
              >
                <span className="flex min-w-0 items-center gap-1.5 whitespace-nowrap">
                  <span className="text-muted-foreground font-mono">[^{proposedIdentifier}]</span>
            ...
            {filteredIdentifiers.map((identifier: string) => (
              ...
              >
                <span className="flex min-w-0 items-center gap-1.5 whitespace-nowrap">
                  <span className="text-muted-foreground font-mono">[^{identifier}]</span>
                  <span className="truncate">
                    : {getFootnotePreviewLabel(footnoteApi.definitionText?.({ identifier }))}
                  </span>
                </span>
              </InlineComboboxItem>
            ))}
