export function HeroEyebrowDots() {
  return (
    <span className="hero-eyebrow-dots">
      <span className="bg-c-red h-3.5 w-3.5"></span>
      <span className="bg-c-orange h-3.5 w-3.5"></span>
      <span className="bg-c-yellow h-3.5 w-3.5"></span>
      <span className="bg-c-green h-3.5 w-3.5"></span>
      <span className="bg-c-cyan h-3.5 w-3.5"></span>
      <span className="bg-c-blue h-3.5 w-3.5"></span>
      <span className="bg-c-purple h-3.5 w-3.5"></span>
      <span className="bg-c-pink h-3.5 w-3.5"></span>
    </span>
  );
}
